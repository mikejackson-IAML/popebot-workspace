...
export function handleMessage(request: WorkerRequest): WorkerResponse {
  try {
    switch (request.type) {
      ...
      case "deleteProject":
        store.deleteProject(request.id); return { type: "ok" };
      ...
      case "deletePhase":
        store.deletePhase(request.id); return { type: "ok" };
      case "reorderPhases":
        store.reorderPhases(request.projectId, request.phaseIds); return { type: "ok" };
      case "createSpec":
      ...
      case "deleteConversationRef":
        store.deleteConversationRef(request.id); return { type: "ok" };
